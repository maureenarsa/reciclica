...
describe('LoginEffects', () => {
    ...
    let user: User = { id: "anyUserId" };  // Proper initialization of `user`

    let authServiceMock = {
        recoverEmailPassword: (email: string) => {
            if (email === "error@email.com") {
                return throwError(() => error);  // Wrap in a function to conform to RxJS 7+ standard
            }
            return of({});
        },
    ...
    beforeEach(() => {
        TestBed.configureTestingModule({
            imports: [
                ...
                EffectsModule.forFeature([LoginEffects]) // Register the effect here
            ],
            providers: [
                provideMockActions(() => actions$),
                { provide: AuthService, useValue: authServiceMock } // Override the AuthService
            ]
        });

        effects = TestBed.inject(LoginEffects); // Use inject instead of get
    });
    ...
    it('should recover password with not existing email and return an error', (done) => {
    ...
    it('should login with invalid credentials and return error', (done) => {  // Corrected `it` syntax
